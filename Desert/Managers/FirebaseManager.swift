...
class FirebaseManager {
    ...
    /// Routes all Firestore writes to `_trips_dev` in DEBUG builds,
    /// and `trips` in production — no manual changes needed.
    private var tripsCollection: String {
        #if DEBUG
        // To test alerts: change "_trips_dev" to "trips" temporarily
    ...
    /// Routes the trip counter to `_meta_dev` in DEBUG builds,
    /// and `meta` in production — keeps dev counters separate.
    private var metaCollection: String {
        #if DEBUG
        return "_meta_dev"
        #else
        return "meta"
