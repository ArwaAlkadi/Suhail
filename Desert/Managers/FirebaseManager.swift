...
class FirebaseManager {
    ...
    /// Controls whether trip data is uploaded to Firebase.
    /// When `false`, no data is written to Firebase at all — trip runs fully offline.
    /// When `true`, data is written to `_trips_dev` — never touches production.
    /// Toggled from `TripSummaryView` in DEBUG builds only — persisted in UserDefaults.
    var isProductionEnabled: Bool {
        get { UserDefaults.standard.bool(forKey: UserDefaultsKeys.isProductionEnabled) }
        set { UserDefaults.standard.set(newValue, forKey: UserDefaultsKeys.isProductionEnabled) }
    }

    // MARK: - Environment

    /// Writes to `_trips_dev` — production (`trips`) is never touched from the app.
    private var tripsCollection: String { "_trips_dev" }

    /// Uses `_meta_dev` — keeps dev counters separate from production.
    private var metaCollection: String { "_meta_dev" }

    // MARK: - Upload Guard

    /// Returns `true` and prints a warning if uploads are disabled.
    /// Call this at the top of every write function.
    private func uploadsDisabled(caller: String = #function) -> Bool {
        guard isProductionEnabled else {
            print("[\(caller)] Upload skipped — Firebase disabled in developer settings")
            return true
        }
        return false
    }
    ...
    ///
    /// When Firebase is disabled (dev toggle off), generates a local-only ID
    /// in the format `trip_dev_8f3k9mp2` without touching Firestore.
    ///
    ...
    func createTripId(completion: @escaping (String) -> Void) {
        guard !uploadsDisabled() else {
            // Generate a local-only trip ID without touching Firebase
            let random = UUID().uuidString
                .replacingOccurrences(of: "-", with: "")
                .prefix(8)
                .lowercased()
            let tripId = "trip_dev_\(random)"
            print("trip ID created locally (Firebase disabled): \(tripId)")
            completion(tripId)
            return
        }

        let counterRef = db.collection(metaCollection).document("tripCounter")
    ...
    func saveTrip(_ trip: Trip, tripId: String) {
        ...
        db.collection(tripsCollection).document(tripId).setData([
    ...
    ///
    /// Writes to `04-lastKnownLocation` only — does not rewrite the full document.
    ...
    ///
    /// Updates `02-status.a-status` to `"completed"` and records the end time.
    ///
    ...
    func listenToTripStatus(
        ...
        db.collection(tripsCollection).document(tripId).addSnapshotListener { snapshot, error in
            if let error {
    ...
    func fetchAlertStatus(
        ...
        db.collection(tripsCollection).document(tripId).getDocument { document, error in
